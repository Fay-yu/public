									...
									<table class="table table-striped table-bordered table-hover">
										...
										<tbody id="all">
											...
											<tr>
												...
												%>
												<%
													if(person.getApplystate()==4) {
												%>
												<td>已完成！<span class="label label-success">Refuse</span>
												</td>
												<%
													}
												%>
												...
												<td>
													<div class="btn-group1">
														<a
															href="/lavaFAQ/pages/user/deleteApply?id=<%=person.getId()%>&userid=<%=person.getUserid()%>"
															onclick="return myclick();"
															style="text-decoration:none; 
color:#333; "> 取消申请
															<button class="btn btn-xs btn-danger"
																value="<%=person.getId()%>"
																id="remove<%=person.getId()%>">
																<input type="hidden" id="<%=person.getId()%>"
																	value="<%=person.getId()%>" /> <i class="icon-remove"></i>
															</button>
														</a>
													</div>
												</td>
												<%
													}else if(person.getApplystate()==2||person.getApplystate()==3){
												%>
												<td>
													<div class="btn-group1">
														<a
															href="/lavaFAQ/pages/user/updateApply?id=<%=person.getId()%>&userid=<%=person.getUserid()%>"
															onclick="return yourclick();"
															style="text-decoration:none; 
color:#333; "> <input
															type="hidden" id="<%=person.getId()%>"
															value="<%=person.getId()%>" /> <span
															class="label label-success">完成此次预约</span>
														</a>
													</div>
												</td>
												<%
